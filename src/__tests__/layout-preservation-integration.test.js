...
import { arrangeImagesFullCover, FULL_COVER_LAYOUT_TYPES } from '../utils/fullCoverLayoutUtils.js'
...
vi.mock('../utils/layoutUtils.js', () => ({
  arrangeImages: vi.fn((images) => images.map((img, index) => ({
    ...img,
    x: index * 150,
    y: 0,
    previewWidth: 150,
...
describe('Layout Preservation Integration Tests', () => {
  ...
  describe('Complete Layout Preservation Workflow', () => {
    it('should preserve flexible layout through image reordering', async () => {
      ...
      const result = await reapplyCurrentLayout(reorderedImages, settings, pageId)
    ...
    it('should maintain different layout types across operations', async () => {
      ...
      const layout1Positions = result1.map(img => `${img.x},${img.y},${img.previewWidth}x${img.previewHeight}`)
      const layout2Positions = result2.map(img => `${img.x},${img.y},${img.previewWidth}x${img.previewHeight}`)
    ...
    it('should handle mixed grid and flexible layouts correctly', async () => {
      ...
      const gridLayouts = results.filter(r => r.layoutType === FULL_COVER_LAYOUT_TYPES.GRID)
      const flexibleLayouts = results.filter(r => r.layoutType === FULL_COVER_LAYOUT_TYPES.FLEXIBLE)
    ...
    it('should preserve layout when adding/removing images', async () => {
      ...
      const withAddedImage = [...originalImages, {
        id: 'img-5',
        naturalWidth: 800,
        naturalHeight: 600
      }]

      const resultWith5 = await reapplyCurrentLayout(withAddedImage, settings, pageId)
      ...
      const resultWith3 = await reapplyCurrentLayout(withRemovedImage, settings, pageId)
  ...
  describe('Error Recovery and Edge Cases', () => {
    ...
    it('should handle extreme image counts correctly', async () => {
      ...
      const resultMany = await reapplyCurrentLayout(manyImages, settings, pageId)
