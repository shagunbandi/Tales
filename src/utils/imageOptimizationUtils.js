 ...
 */
export const getOptimalPrintDimensions = (pageSize = "a4", orientation = "landscape") => {
  const pageDimensions = PAGE_SIZES[pageSize];
  const DPI = 300;
  const MM_TO_INCH = 25.4;

  // Get page dimensions in mm
  const pageWidthMm = orientation === "landscape" ? pageDimensions.width : pageDimensions.height;
  const pageHeightMm = orientation === "landscape" ? pageDimensions.height : pageDimensions.width;
 ...
 */
export const optimizeImageForPDF = async (imageSrc, targetWidth, targetHeight, options = {}) => {
 ...
 */
export const getOptimalAreaDimensions = (pageWidth, pageHeight, areaWidth, areaHeight, previewWidth, previewHeight) => {
 ...
 */
export const batchOptimizeImagesForPDF = async (images, pageSettings, previewDimensions, onProgress = null) => {
