...
class IndexedDBStorage {
    ...
    return new Promise((resolve, reject) => {
      ...
      request.onupgradeneeded = (event) => {
        ...
        if (!db.objectStoreNames.contains(STORAGE_CONFIG.STORES.ALBUMS)) {
          const albumStore = db.createObjectStore(STORAGE_CONFIG.STORES.ALBUMS, { keyPath: 'id' });
        ...
        if (!db.objectStoreNames.contains(STORAGE_CONFIG.STORES.THUMBNAILS)) {
          db.createObjectStore(STORAGE_CONFIG.STORES.THUMBNAILS, { keyPath: 'albumId' });
        }
      };
    });
  }
  ...
  async saveAlbum(albumData) {
    ...
    const transaction = this.db.transaction([STORAGE_CONFIG.STORES.ALBUMS], 'readwrite');
  ...
  async getAlbum(albumId) {
    ...
    const transaction = this.db.transaction([STORAGE_CONFIG.STORES.ALBUMS], 'readonly');
    const store = transaction.objectStore(STORAGE_CONFIG.STORES.ALBUMS);
    ...
    return new Promise((resolve, reject) => {
      ...
      request.onsuccess = async () => {
        if (request.result) {
          const decompressedAlbum = await this.decompressAlbumData(request.result);
    ...
    for (let pageIndex = 0; pageIndex < albumData.pages.length; pageIndex++) {
      ...
      for (const originalImage of originalPage.images) {
        ...
        if (originalImage.src && originalImage.src.startsWith('data:')) {
          const compressedImage = await this.compressImageLossless(originalImage.src);
          imagesToSave.push({ id: imageId, blob: compressedImage });
          
          // Handle original (uncropped) version if present
          if (originalImage.originalSrc && originalImage.originalSrc.startsWith('data:')) {
            const originalId = `${imageId}_original`;
            const compressedOriginal = await this.compressImageLossless(originalImage.originalSrc);
  ...
  async decompressAlbumData(albumData) {
    ...
    for (const page of album.pages) {
      for (const image of page.images) {
        ...
        if (image.originalImageRef) {
          try {
            const originalBlob = await this.getImageBlob(image.originalImageRef);
            image.originalSrc = await this.blobToDataURL(originalBlob);
          } catch (error) {
            console.warn(`Failed to load original image ${image.originalImageRef}:`, error);
            image.originalSrc = '';
          }
        }
    ...
    for (const image of album.availableImages) {
      if (image.imageRef) {
        try {
          ...
          console.warn(`Failed to load available image ${image.imageRef}:`, error);
          image.src = '';
        }
      }
  ...
  async saveImagesBatch(imagesToSave) {
    ...
    for (let i = 0; i < imagesToSave.length; i += CHUNK_SIZE) {
      ...
      const transaction = this.db.transaction([STORAGE_CONFIG.STORES.IMAGES], 'readwrite');
      ...
      await new Promise((resolve, reject) => {
        ...
        for (const imageData of chunk) {
          try {
            const request = store.put({ id: imageData.id, blob: imageData.blob });
  ...
  async getImageBlob(imageId) {
    const transaction = this.db.transaction([STORAGE_CONFIG.STORES.IMAGES], 'readonly');
    const store = transaction.objectStore(STORAGE_CONFIG.STORES.IMAGES);
...
export class StorageManager {
  ...
  async cleanupOrphanedImagesWeb() {
    ...
    for (const album of albums) {
      for (const page of album.pages) {
        for (const image of page.images) {
          ...
          if (image.originalImageRef) activeImageRefs.add(image.originalImageRef);
        }
      }
      for (const image of album.availableImages) {
        if (image.imageRef) activeImageRefs.add(image.imageRef);
    ...
    const transaction = this.indexedDB.db.transaction([STORAGE_CONFIG.STORES.IMAGES], 'readonly');
    const store = transaction.objectStore(STORAGE_CONFIG.STORES.IMAGES);
    ...
    return new Promise((resolve) => {
      ...
      request.onsuccess = async () => {
        ...
        const orphanedImages = allImageIds.filter(id => !activeImageRefs.has(id));
