...
export class CapacitorStorage {
  ...
  async saveAlbum(albumData) {
    ...
    try {
      ...
      const processedAlbum = await this.processAlbumForStorage(albumData, albumDir);
  ...
  async processAlbumForStorage(albumData, albumDir) {
    ...
    for (let pageIndex = 0; pageIndex < album.pages.length; pageIndex++) {
      ...
      for (let imgIndex = 0; imgIndex < page.images.length; imgIndex++) {
        ...
        if (image.src && image.src.startsWith('data:')) {
          ...
          const filepath = await this.saveImageFile(image.src, albumDir, filename);
          image.src = filepath;
        }
        if (image.originalSrc && image.originalSrc.startsWith('data:')) {
          ...
          const filepath = await this.saveImageFile(image.originalSrc, albumDir, filename);
    ...
    for (let imgIndex = 0; imgIndex < album.availableImages.length; imgIndex++) {
      ...
      for (const image of page.images) {
        ...
        if (image.originalSrc && !image.originalSrc.startsWith('data:')) {
          try {
            ...
            console.error('Error loading original image:', image.originalSrc, error);
            image.originalSrc = '';
          }
        }
      }
  ...
  async deleteAlbum(albumId) {
    try {
      ...
      try {
        ...
        console.warn('Album directory might not exist or already deleted:', error);
