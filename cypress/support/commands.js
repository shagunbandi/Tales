...
Cypress.Commands.add("uploadTestImages", (imageCount = 3) => {
  ...
  cy.get('[data-testid="processing-indicator"]', { timeout: 10000 }).should(
    "not.exist",
  );
});

Cypress.Commands.add(
  "uploadSpecificTestImage",
  (width, height, color = "#ff0000", text = "Test") => {
    cy.get('input[type="file"]').then((input) => {
      const canvas = document.createElement("canvas");
      canvas.width = width;
...
Cypress.Commands.add("configureSettings", (settings) => {
  ...
  if (settings.pageMargin) {
    cy.get('[data-testid="page-margin-input"]')
      .clear()
      .type(settings.pageMargin.toString());
  }

  if (settings.imageGap) {
    cy.get('[data-testid="image-gap-input"]')
      .clear()
      .type(settings.imageGap.toString());
  }

  if (settings.maxImagesPerRow) {
    cy.get('[data-testid="max-images-per-row-input"]')
      .clear()
      .type(settings.maxImagesPerRow.toString());
  }

  if (settings.maxNumberOfRows) {
    cy.get('[data-testid="max-number-of-rows-input"]')
      .clear()
      .type(settings.maxNumberOfRows.toString());
  }
...
Cypress.Commands.add(
  "dragImageBetweenPages",
  (fromPageIndex, imageIndex, toPageIndex) => {
...
Cypress.Commands.add("waitForPDFGeneration", () => {
  cy.get('[data-testid="pdf-generating"]', { timeout: 30000 }).should(
    "not.exist",
  );
});
...
Cypress.Commands.add("verifySettingsValue", (setting, expectedValue) => {
  ...
  cy.get(settingSelectors[setting]).should(
    "have.value",
